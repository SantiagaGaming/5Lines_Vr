...
public class OscilButton : BaseButtton
{
    ...
    private IEnumerator MoveButton(bool down)
    {
        if(down)
        {
            ...
            while(_buttonBox.transform.localPosition.y>=0.45f)
            {
                _buttonBox.transform.localPosition -= new Vector3(0, 0.005f, 0);
        ...
        else
        {
            ...
            while (_buttonBox.transform.localPosition.y <= 0.55f)
            {
                _buttonBox.transform.localPosition += new Vector3(0, 0.005f, 0);
                yield return new WaitForSeconds(0.005f);
            }
